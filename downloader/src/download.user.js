...
// @author       Codex
// @updateURL    https://sgs.senrax.com/script/download.mjs
// @downloadURL  https://sgs.senrax.com/script/download.mjs
...
(function sgsXcDownloader() {
  ...
  const config = {
    frameworkUrl: "https://sgs.senrax.com/script/core.mjs",
    fallbackFrameworkUrl: "https://raw.githubusercontent.com/suujonH/sgs-xc/main/framework/core.mjs",
    timeoutMs: 5000,
  ...
  async function boot() {
    ...
    try {
      ...
      const { url: frameworkUrl, response } = await requestFrameworkWithFallback(cached);
      const remote = {
        url: frameworkUrl,
  ...
  async function requestFrameworkWithFallback(cached) {
    const urls = [config.frameworkUrl, config.fallbackFrameworkUrl].filter(Boolean);
    const errors = [];
    for (const url of urls) {
      try {
        const response = await requestFramework(url, config.timeoutMs, cached.meta);
        if (response.status === 304 && !cached.code) {
          throw new Error(`${url} returned 304 but no cached code exists.`);
        }
        return { url, response };
      } catch (error) {
    ...
    throw new Error(`All Framework sources failed:\n${errors.join("\n")}`);
  }
  ...
  function extractFrameworkVersion(code, sha256) {
    ...
    const match = source.match(/^\/\/\s*@version\s+([^\s]+)\s*$/m)
      || source.match(/\bconst\s+version\s*=\s*["']([^"']+)["']/);
  ...
  function showFailureMessageBox(errorText, cachedMeta) {
    return new Promise((resolve) => {
      const render = () => {
        ...
        body.textContent = [
          "两个 Framework 下载来源均失败（每个请求超时 5 秒）。",
