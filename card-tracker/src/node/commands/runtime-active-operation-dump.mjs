...
function outputDir() {
  return path.resolve(
    process.env.SGS_ACTIVE_OPERATION_DUMP_DIR ||
      path.join(projectRoot, "work", "runtime-exploration", `${timestampName()}-active-operation-dump`)
  );
}
...
function readmeText(payload, rows) {
  ...
  return [
    "# Runtime Active Operation Dump",
    "",
    `- Captured: ${new Date().toISOString()}`,
    `- Scene: ${status.scene?.scene || ""}`,
    `- Game over: ${status.scene?.isGameOver ?? ""}`,
    `- Records: ${rows.length}`,
    `- Calls: ${activeRows.length}`,
    `- Send/confirm-like calls: ${sendRows.length}`,
    `- Samples: ${status.sampleCount || 0}`,
    `- Wrappers still installed: ${status.wrapperCount || 0}`,
    `- Blocked purchase-like calls: ${status.blockedCalls || 0}`,
    `- Errors: ${status.errors?.length || 0}`,
    "",
    "This command only dumps the already-installed passive recorder. It keeps the recorder installed and does not click, confirm, use, discard, play, buy, refresh, pay, or read hidden opponent hand fields.",
    "",
    "## Top Labels",
    "",
...
async function main() {
  ...
  const payload = { target: dumpResult.target, dump: dumpResult.value };
  const rows = rowsFromRecords(payload.dump?.records || []);
  await writeJson(path.join(dir, "active-operation-dump.json"), payload);
  ...
  console.log(JSON.stringify({
    ...
    gameOver: payload.dump?.status?.scene?.isGameOver ?? "",
    records: rows.length,
    ...
    wrappersStillInstalled: payload.dump?.status?.wrapperCount || 0,
    recorderStillInstalled: payload.dump?.status?.installed === true
  }, null, 2));
}
